...
export function PageShell({
  children,
  background = PHOTOS.panorama,
}: {
  children: ReactNode;
  background?: string;
}) {
  return (
    <div className="page-light relative flex min-h-screen flex-col bg-background">
  ...
  return (
    <section className="relative overflow-hidden border-b border-border/60">
      <img
        src={image}
        alt=""
        className="absolute inset-0 size-full scale-105 object-cover animate-in fade-in duration-1000"
      />
      ...
      <div className="relative mx-auto w-full max-w-4xl px-4 py-16 text-center sm:px-6 sm:py-20 lg:py-28 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-sm text-foreground/80 sm:text-base lg:text-lg">
          {copy}
        </p>
        {children ? (
